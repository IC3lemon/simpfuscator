...
class Obfuscator:
    ...
    def ghost_child_obfuscate(self, option, key=None):
        ...
        func_sign = {
            ...
            2 : f'decrypt_rsa(elf_bytes, elf_len, {key[0]}, {key[1]}, {key[2]})',
            3 : f'decrypt_aes(elf_bytes, elf_len, aes_key, aes_iv)'
        }
...
'''
        success, stdout, stderr, op = compile_c_string(
            c_code,
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(f"Usage: {sys.argv[0]} <elf_binary> <output_path> <option>")
        sys.exit(1)
    # option 1 -> xor
    # option 2 -> rsa
    # option 3 -> aes
    ...
    obfuscator.ghost_child_obfuscate(1)
